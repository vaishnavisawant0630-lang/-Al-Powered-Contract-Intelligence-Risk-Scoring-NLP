...
class TextExtractor:
    ...
    def extract(self, path: Path) -> ExtractionResult:
        ...
        for enc in self._ENCODING_FALLBACK_CHAIN:
            try:
        ...
        if text is None:
            raise ExtractionError(
                f"Failed to decode {path} with any of {self._ENCODING_FALLBACK_CHAIN}: {last_error}"
            )

        if encoding_used == "latin-1":
            log.warning("non-UTF8 encoding detected for %s — decoded as latin-1", path)
